...
fn render_fixture_matches_library_golden() {
    ...
    let message = parse_message(&json).unwrap().message;
    assert_eq!(
        stdout,
...
#[test]
fn oversized_content_renders_successfully_because_rendering_stays_permissive() {
    ...
    assert!(
        output.status.success(),
        "render must not run the validate gate, exit code {:?}",
        output.status.code()
    );
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(
        stdout.contains(&format!("<p>{}</p>", "x".repeat(2001))),
        "all 2001 characters render"
    );
}
